...
## 右下角圆形「音乐」入口；展开后：曲名、进度、暂停、上/下一首、音乐音量（与设置同步，内置 BGM）
...
func _ready() -> void:
	...
	GameSettings.music_linear_changed.connect(_on_music_linear_sync)
	_build_ui()
...
func _on_music_linear_sync(v: float) -> void:
	if _vol_slider == null:
		return
